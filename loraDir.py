...
def checkcollision(packet):
    ...
    for i in range(0,len(packetsAtBS)):
        if packetsAtBS[i].packet.processed:
...
# is maintained
#
#dar uma olhada na saída dessa função
def transmit(env,node):
    while True:
        ...
        if (node in packetsAtBS):
            ...
            node.packet.processed = True

            # global nrProcessed
            # nrProcessed = nrProcessed + 1
        #esta parte verifica qual a classificação do pacote para fazer a contagem dos resultados
        #dar uma olhada nesses valores e talvez colocar um if dentro do outro
        #com certeza algum desses valores esta errado
        if node.packet.lost:
            global nrLost
            nrLost += 1
        else:
            if node.packet.collided:
                global nrCollisions
                nrCollisions += 1
            else:
                global nrReceived
...
# uncomment next line to turn on graphics
# graphics = True

# uncomment next line to do the full collision check
# full_collision = True

nrNodes = 100
avgSendTime = 10#1200000
experiment = int(sys.argv[1])
simtime = 1000#24*3600*1000
...
maxDist = 1000 #d0*(math.e**((Lpl-Lpld0)/(10.0*gamma)))
...
der2 = 0
if sent != 0:
    der = (sent-nrCollisions-nrLost)/float(sent)
    print "DER:", der
    der2 = (nrReceived)/float(sent)
    print "DER method 2:", der2

if (der!=der2):
    raw_input('DER ERROR. Press Enter to continue ...')
...
if os.path.isfile(fname):
    res = "\n" + str(nrNodes) + " "  + str(sent) + " " + str(nrReceived) + " " + str(nrLost) + " "  + str(nrCollisions) + " " + str(energy) + " " + str(der)
else:
    res = "#nrNodes nrTransmissions nrReceived nrLost nrCollisions OverallEnergy DER\n" + str(nrNodes) + " "  + str(sent) + " " + str(nrReceived) + " " + str(nrLost) + " " + str(nrCollisions) + " " + str(energy) + " " + str(der)
...
with open('nodes.txt','w') as nfile:
    for n in nodes:
        nfile.write("{} {} {} {}\n".format(n.x, n.y, n.nodeid, n.packet.sf))
