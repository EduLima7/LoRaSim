...
if (cmdMode):
    parameters = ["Experiment", "NrNodes", "NrBS", "BaseDist", "NrNetworks", "AvgSendTime", "Simtime", "Full_collision", "Directionality"]
    answers = []
    for i in parameters:
        input_parameter = input(i+": ")
        answers.append(input_parameter)

    experiment = answers[0]
    nrNodes = answers[1]
    nrBS = answers[2]
    baseDist = answers[3]
    nrNetworks = answers[4]
    avgSendTime = answers[5]
    simtime = bool(int(answers[6]))
    full_collision = answers[7]
    directionality = answers[8]
